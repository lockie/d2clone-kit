...
#+BEGIN_SRC lisp :tangle components.lisp :noweb yes :exports none
<<preamble>>
...
=components.lisp= file contains a couple of advanced macroses used to define a new component, as well as few component-related generics belonging to systems (described in the next section) and a couple of small component-related helpers.

It is worth mentioning that in our engine we have one-to-zero-or-one relation between component and system (i.e. the component type always belongs to a singular system, and the system can have only one component type or no associated component type at all).

First, we define ~make-component~ generic that takes a system instance, an entity and component creation parameters and creates a component for the system belonging to the given entity:
...
The ~&rest~ parameters plist could include ~:prefab~ key, in which case it specifies that the component should be created from the given prefab. Prefabs are covered in depth in [[Prefabs]] section.

Next, we define ~delete-component~ generic that takes a system instance and an entity, and deletes the component from the given entity:

#+BEGIN_SRC lisp :noweb-ref delete-component
(defgeneric delete-component (system entity)
  (:documentation "Deletes SYSTEM's component from ENTITY."))

#+END_SRC

We also define the default ~delete-component~ method for componentless systems, which does nothing:
...
Next, we have the helper that's used in components memory management, called ~system-adjust-components~, which adjusts the size of system component storage described in detail in the next secion:

#+NAME: system-adjust-components
#+BEGIN_SRC lisp
(declaim
...
Next, we got a couple of helpers used to determine whether the entity has some system's component: the ~%has-component-p~ does the test for given system instance and entity without any extra checks:

#+NAME: %has-component-p
#+BEGIN_SRC lisp
(declaim
...
Its twin brother, ~has-component-p~, takes the ~system~ argument as a keyword denoting the system, and it also bothers itself with actually checking if the system is already created, making it useful in cases of early system initialization:
...
Next, we got this huge internal ~defsoa~ macro that is used to define a structure of arrays --- a container for components we're using to leverage CPU caching. I won't go into detail of that macro, only stating that it defines an actual structure in ~defstruct~ sense with growable arrays as its fields, and a bunch of accessors, both safe and unsafe (the former using ~growable-vector-ref*~ described in detail in [[Growable vectors]]).
...
Finally, we implement ~defcomponent~ macro used to define a new component type within a given system referenced by ~system~ symbol. One can also specify a name for the component, which is by default is set to the system name. The macro also takes ~slots~ argument which behaves similarly to the ~defstruct~ slot description argument.
